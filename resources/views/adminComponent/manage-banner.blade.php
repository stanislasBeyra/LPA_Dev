...
@section('content')
    <div class="container-fluid pt-4">
        ...
        <div class="d-flex justify-content-between mb-3">
            ...
            <!-- Bouton à droite -->
            <button type="button" class="btn btn-primary fs-7 animate__animated animate__zoomIn" data-mdb-ripple-init
                data-mdb-modal-init data-mdb-target="#addCategoryModal">
                Add banners
            </button>
        </div>
    ...
    @enderror
                            ...
                            <div class="form-outline mb-4">
                                ...
                                <input style="border: none;" type="file" id="banner_image" name="banner_image"
                                    class="form-control d-none @error('banner_image') is-invalid @enderror" accept="image/*"
                            ...
                            <div class="card d-none" id="Img-card">
                                <img id="image_preview" src="#" alt="Image Preview"
                                    style="height: 500px;object-fit:cover;" class="card-img-top img-fluid" />
                            </div>
                        </div>
        ...
        <div class="row">
            @foreach ($banners as $banner)
                @php
                    $imageUrl = Upload::getAbosoluteUrl('banners', $banner->image_url);
                @endphp
                <div class="col-md-3 mb-4">
                    <div class="card">
                        <img src="{{ $imageUrl }}" class="card-img-top" alt="Banner Image" />

                        <div class="card-body d-flex align-items-center justify-content-between">
                            <div class="form-check form-switch me-3">
                                <input class="form-check-input banner-status-switch" type="checkbox" role="switch"
                                    id="bannerSwitch{{ $banner->id }}" data-banner-id="{{ $banner->id }}"
                                    {{ $banner->is_active ? 'checked' : '' }} />
                            </div>
                            <button type="button" class="btn btn-danger btn-sm" data-mdb-modal-init
                                data-mdb-target="#exampleModal1" onclick="setBanner('{{ $banner->id }}')">
                ...
                switchElement.addEventListener('change', function() {
                    ...
                    fetch("{{ route('banner.status.update') }}", {
                        ...
                        .then(data => {
                            if (data.success) {
                                // Afficher le toast de succès
                                document.getElementById('successToastBody').textContent = data
                                    .message;
                                var successToast = new mdb.Toast(document.getElementById(
                                    'successToast'));
                                successToast.show();
                            } else {
                                ...
                                // Afficher le toast d'erreur
                                document.getElementById('errorToastBody').textContent = data
                                    .message;
                                var errorToast = new mdb.Toast(document.getElementById(
                                    'errorToast'));
                                errorToast.show();
                            }
                        })
